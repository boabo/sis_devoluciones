CREATE OR REPLACE FUNCTION segu.execute_transaction_from_pxp_nd(
    p_params json
)
    RETURNS json AS
$body$
...
BEGIN
    ...
    FOR v_parameters
        IN (
            SELECT *
            FROM json_populate_recordset(NULL::record, v_create_parameters::json)
                     AS
                     (
    ...
    v_transaction_string_for_execute :=
                                                    'select ' || v_procedure || '('||v_is_admin||',' || coalesce(v_user_id, 0) || ',''' || v_tabla ||
                                                    ''',''' || v_transaction || ''')';
