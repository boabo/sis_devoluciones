CREATE OR REPLACE FUNCTION decr.f_dosi_siguiente (
  json json
)
RETURNS text AS
$body$
...
BEGIN
	v_nombre_funcion = 'decr.f_dosi_siguiente';

      IF EXISTS (SELECT 0 FROM decr.tdosi_correlativo where id_dosificacion = cast(json->>'id_dosificacion' as int) )
    THEN
      --stuff here


        select nro_siguiente into v_nro_siguiente from decr.tdosi_correlativo where id_dosificacion = cast(json->>'id_dosificacion' as int);

      ELSE

      insert into decr.tdosi_correlativo(
